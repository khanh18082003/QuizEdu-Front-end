...
const EditMatchingModal: React.FC<EditMatchingModalProps> = ({
  ...
  const updateQuestion = (
    index: number,
    field: keyof UpdateMatchingQuestionByType,
    value: any,
  ) => {
  ...
  const handleFileChange = (
    index: number,
    field: "file_content_a" | "file_content_b",
    file: File | null,
  ) => {
    if (file) {
  ...
  const validateForm = () => {
    ...
    questions.forEach((question, index) => {
      ...
      if (question.type_a === "TEXT" && !question.content_a?.trim()) {
        ...
        question.type_a === "IMAGE" &&
        !question.file_content_a &&
        !filePreviews[`${index}-file_content_a`]
      ) {
        newErrors[`${index}-content_a`] =
          "Vui lòng chọn file hình ảnh cho nội dung A";
      }
      ...
      if (question.type_b === "TEXT" && !question.content_b?.trim()) {
        ...
        question.type_b === "IMAGE" &&
        !question.file_content_b &&
        !filePreviews[`${index}-file_content_b`]
      ) {
        newErrors[`${index}-content_b`] =
          "Vui lòng chọn file hình ảnh cho nội dung B";
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm">
      <div className="max-h-[95vh] w-full max-w-7xl overflow-hidden rounded-xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-800">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-200 bg-gradient-to-r from-green-50 to-blue-50 p-6 dark:border-gray-700 dark:from-green-900/20 dark:to-blue-900/20">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-r from-green-500 to-blue-500">
              <FaArrowRightArrowLeft className="text-lg text-white" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                {presetType
                  ? `Chỉnh sửa câu hỏi ghép đôi - ${
                      presetType.typeA === "TEXT" && presetType.typeB === "TEXT"
                        ? "📝 Văn bản - Văn bản"
                        : presetType.typeA === "TEXT" &&
                            presetType.typeB === "IMAGE"
                          ? "📝🖼️ Văn bản - Hình ảnh"
                          : presetType.typeA === "IMAGE" &&
                              presetType.typeB === "TEXT"
                            ? "🖼️📝 Hình ảnh - Văn bản"
                            : "🖼️ Hình ảnh - Hình ảnh"
                    }`
                  : "Chỉnh sửa câu hỏi ghép đôi"}
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {presetType ? (
                  <>
                    Chỉnh sửa cặp ghép{" "}
                    <strong>
                      {presetType.typeA}-{presetType.typeB}
                    </strong>{" "}
                    với <strong>{presetType.points} điểm/cặp</strong>
                  </>
                ) : (
        ...
        <div className="max-h-[calc(95vh-180px)] overflow-y-auto p-6">
          ...
          <div className="space-y-8">
            {questions.map((question, index) => (
              <div
                key={`question-${index}`}
                className="rounded-xl border border-gray-200 bg-gradient-to-br from-gray-50 to-gray-100 p-6 shadow-sm transition-all duration-200 hover:shadow-md dark:border-gray-600 dark:from-gray-700/50 dark:to-gray-800/50"
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {!expandedQuestions[index] && (
                      <div className="text-sm text-gray-500 dark:text-gray-400">
                        ({question.type_a} - {question.type_b}) •{" "}
                  ...
                  <>
                    <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-3">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <label className="text-sm font-medium tracking-wide text-blue-600 uppercase dark:text-blue-400">
                            Nội dung A
                          </label>
                          <select
                            ...
                            className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1 text-sm font-medium text-blue-800 disabled:opacity-50 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-200"
                        ...
                        {question.type_a === "TEXT" ? (
                          <div>
                            <textarea
                              ...
                              className={`w-full resize-none rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-gray-900 placeholder-gray-500 transition-all focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-blue-800 dark:bg-blue-900/20 dark:text-white dark:placeholder-gray-400 ${
                          ...
                          <div>
                            <input
                              ...
                              className={`w-full rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-gray-900 transition-all file:mr-4 file:rounded-lg file:border-0 file:bg-blue-100 file:px-4 file:py-2 file:text-sm file:font-medium file:text-blue-700 hover:file:bg-blue-200 dark:border-blue-800 dark:bg-blue-900/20 dark:text-white dark:file:bg-blue-800 dark:file:text-blue-200 ${
                                errors[`${index}-content_a`]
                                  ? "border-red-300 bg-red-50 dark:bg-red-900/20"
                                  : ""
                              }`}
                            ...
                            {filePreviews[`${index}-file_content_a`] && (
                              <div className="mt-3">
                                <img
                                  ...
                                  alt="Preview A"
                                  className="h-32 w-full max-w-xs rounded-lg border border-blue-200 object-cover dark:border-blue-600"
                                />
                                <p className="mt-1 text-xs text-gray-500">
                                  {filePreviews[
                                    `${index}-file_content_a`
                                  ].startsWith("blob:")
                                    ? "Hình ảnh mới được chọn"
                                    : "Hình ảnh hiện tại"}
                                </p>
                              </div>
                            )}
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <label className="text-sm font-medium tracking-wide text-purple-600 uppercase dark:text-purple-400">
                            Nội dung B
                          </label>
                          <select
                            ...
                            className="rounded-lg border border-purple-200 bg-purple-50 px-3 py-1 text-sm font-medium text-purple-800 disabled:opacity-50 dark:border-purple-800 dark:bg-purple-900/20 dark:text-purple-200"
                            disabled={isLoading || presetType !== null}
                          >
                            <option value="TEXT">📝 Văn bản</option>
                            <option value="IMAGE">🖼️ Hình ảnh</option>
                        ...
                        {question.type_b === "TEXT" ? (
                          <div>
                            <textarea
                              ...
                              className={`w-full resize-none rounded-lg border border-purple-200 bg-purple-50 px-4 py-3 text-gray-900 placeholder-gray-500 transition-all focus:border-transparent focus:ring-2 focus:ring-purple-500 dark:border-purple-800 dark:bg-purple-900/20 dark:text-white dark:placeholder-gray-400 ${
                            ...
                            {filePreviews[`${index}-file_content_b`] && (
                              <div className="mt-3">
                                <img
                                  ...
                                  alt="Preview B"
                                  className="h-32 w-full max-w-xs rounded-lg border border-purple-200 object-cover dark:border-purple-600"
                                />
                                <p className="mt-1 text-xs text-gray-500">
                                  {filePreviews[
                                    `${index}-file_content_b`
                                  ].startsWith("blob:")
                                    ? "Hình ảnh mới được chọn"
                                    : "Hình ảnh hiện tại"}
                                </p>
                    ...
                    <div className="mt-6">
                      <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        {presetType && presetType.minRequired >= 2 && (
                          <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">
                            (Thay đổi điểm sẽ áp dụng cho tất cả cặp trong loại
                            này)
                          </span>
                        )}
                      </label>
                      <input
                        ...
                        min="1"
                        max="100"
                        className={`w-32 rounded-lg border border-gray-300 bg-gray-50 px-4 py-2 text-gray-900 transition-all focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white ${
                          errors[`${index}-points`]
                            ? "border-red-300 bg-red-50 dark:bg-red-900/20"
                            : ""
                        }`}
